...
(if (and emacs/>=31p (fboundp 'treesit-available-p)
         (treesit-available-p))
  ...
  ;; M-x treesit-install-language-grammar twice for libtree-sitter-markdown.so & libtree-sitter-markdown-inline.so
  (use-package markdown-ts-mode
    :mode ("\\.md\\'" "\\.mdx\\'" "\\.markdown\\'")
    :config
