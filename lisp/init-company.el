            ...
            (with-no-warnings
              ...
              (defun my-company-box-doc--set-frame-position (frame)
                (-let* ((frame-resize-pixelwise t)
                        ...
                        (border (or (alist-get 'internal-border-width company-box-doc-frame-parameters) 0))


                        (x (- (+ (car box-position) box-width) border))
                        (space-right (- (frame-pixel-width) x))
                        (space-left (car box-position))
                        (fringe-left (or (alist-get 'left-fringe company-box-doc-frame-parameters) 0))
                        (fringe-right (or (alist-get 'right-fringe company-box-doc-frame-parameters) 0))
                        (real-width (+ width border fringe-left fringe-right))
                        (x (or (and (> real-width space-right)
                                    (> space-left real-width)
                                    (- space-left real-width))
                               x))

                        (y (cdr box-position))
                        (bottom (+ company-box--bottom (window-pixel-top) (frame-border-width)))
                        (y (if (> (+ y height border (* 2 (frame-char-height))) bottom)
                               (- bottom height (frame-border-width) border (* 2 (frame-char-height)))
                             y)))
